        ...
        <div className="w-full max-w-md">
          <div className="flex flex-col gap-6">
            <div className="animate-element animate-delay-50 flex items-center gap-3">
              <img className="w-9 h-9" src="/logo.webp" srcSet="/logo.webp 128w" sizes="36px" alt="Nexnetra logo" />
              <span className="text-lg font-bold text-foreground">Nexnetra</span>
            </div>
            <h1 className="animate-element animate-delay-100 text-4xl md:text-5xl font-semibold leading-tight">{title}</h1>
            <p className="animate-element animate-delay-200 text-muted-foreground">{description}</p>

            {error && <p className="animate-element animate-delay-250 text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-2xl p-3">{error}</p>}
              ...
              <div className="animate-element animate-delay-400">
                ...
                <GlassInputWrapper>
                  <div className="flex items-center">
                    <input name="password" type={showPassword ? 'text' : 'password'} placeholder="Enter your password" className="w-full bg-transparent text-sm p-4 rounded-2xl focus:outline-none" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="flex items-center px-3 shrink-0">
